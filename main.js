...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- 12. Müller-Brockmann Grid System Overlay & Optical Alignment ---
    // Dynamic grid overlay column generator
    document.querySelectorAll('.guides .cols').forEach(colsContainer => {
        const colsCount = parseInt(getComputedStyle(document.documentElement).getPropertyValue('--cols').trim() || '12', 10);
    ...
    function setGrid(on) {
        ...
        if (gridBtn) {
            ...
            const lbl = gridBtn.querySelector('.lbl');
    ...
    document.addEventListener('keydown', (e) => {
        if ((e.key === 'g' || e.key === 'G') && !e.metaKey && !e.ctrlKey && !e.altKey) {
            // Ignore keypress if typing in diary input or chat input
    ...
    // Optical typographic alignment to align visual ink (not layout boxes) to grid lines
    (function() {
        const canvas = document.createElement('canvas');
        const ctx = canvas.getContext('2d');
        const alignSelectors = '.slogan, .win-title, .book-title, .logo-icon';
        
        function alignInk() {
            document.querySelectorAll(alignSelectors).forEach(el => {
                el.style.marginLeft = '0px';
                const text = (el.textContent || '').trim().charAt(0);
                if (!text) return;
                
                const style = getComputedStyle(el);
                let charToMeasure = text;
                if (style.textTransform === 'uppercase') charToMeasure = text.toUpperCase();
                
                ctx.font = `${style.fontStyle} ${style.fontWeight} ${style.fontSize} ${style.fontFamily}`;
                ctx.textAlign = 'left';
                
                const metrics = ctx.measureText(charToMeasure);
                const sideBearing = metrics.actualBoundingBoxLeft; // positive means ink overhangs left
                if (isFinite(sideBearing)) {
                    // Nudge element to visually line up its ink boundary with the column edge
                    el.style.marginLeft = `${sideBearing.toFixed(2)}px`;
                }
            });
        }
        ...
        window.addEventListener('resize', () => {
            ...
            resizeTimeout = setTimeout(alignInk, 120);
        });
    })();
